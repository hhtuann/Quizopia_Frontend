  ...
  return (
    <div>
      ...
      <StudentSearchSelect
        ...
        onResult={(r) =>
          setNotice({ kind: "success", message: `Added ${r.added} student${r.added === 1 ? "" : "s"}.${r.duplicated.length > 0 ? ` ${r.duplicated.length} already members.` : ""}${r.invalid.length > 0 ? ` ${r.invalid.length} invalid.` : ""}` })
        }
      />
